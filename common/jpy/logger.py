...
"""
Log 模块\n
支持 Python2.x 和 Python3.x\n
Python2.x 中文输出到屏幕会乱码，输出到日志可正常，同一句中英文混打会有问题
"""
...
class Logger(object):
    ...
    def __log__(self, level, content, fore_color=COLOR_WHITE, back_color=COLOR_BLACK):
        ...
        if self.__to_screen__:
            ...
            # Python2.x 中英文混打会输出屏幕异常
            try:
                print(log_info)
            except:
                print('The output screen is abnormal, Python2.x does not support mixed Chinese and English sentences')
            self.__set_cmd_default_color__()

        if self.__to_file__ is True:
            with codecs.open('{}.log'.format(self.__file_path__), 'a', 'utf-8') as file_handle:
                file_handle.write('{}\n'.format(log_info))
