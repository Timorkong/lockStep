...
using AIParadoxNotion.Serialization.FullSerializer.Internal;
using UnityEngine;
...
namespace AIParadoxNotion.Serialization
{
    ...
    public static class JSONSerializer
    {
        ...
        public static void SaveAITreeData(string json, string fileName = "", int treeID = 1)
        {
            var prettyJson = PrettifyJson(json);
            string path = Application.dataPath + "/Game/Resources/bin/res/config/ai/";
            var dataPath = path + "AI_" + treeID + ".json";
            File.WriteAllText(dataPath, prettyJson);
            string serverAIPath = "G:\\workspace\\jjsg_dnf\\server\\xserver\\conf\\doc\\AI\\";
            if (System.IO.Directory.Exists(serverAIPath))
            {
                dataPath = serverAIPath + "AI_" + treeID + ".json";
                File.WriteAllText(dataPath, prettyJson);
            }
            Process.Start(dataPath);
        }

        public static void SaveAITreeProto<T>(T proto, int treeID) where T : IMessage<T>
        {
            string filePath = Regex.Replace(Application.dataPath, @"(/unity/Assets)$", "") + "/proto/output/" +"ai/"+ "AI_" + treeID + ".bin";
            ...
            using (MemoryStream memoryStream = new MemoryStream())
            {
                ...
                fileStream.Close();
                fileStream.Dispose();
                //UnityEditor.EditorUtility.DisplayDialog("提示", $"导出成功 {filePath}", "确定");
            }
