...
namespace AIParadoxNotion.Design
{
	...
	partial class EditorUtils
    {
        ...
        public static void LabelField(string info, int fontSize = 10,  bool wordWrap = false)
        {
            if ( string.IsNullOrEmpty(info) == false ) 
            {
                GUIStyle bb = new GUIStyle();
                bb.wordWrap = wordWrap;
                bb.fontSize = fontSize;
                bb.normal.textColor = new Color(1,1,1);   //设置字体颜色的
                EditorGUILayout.LabelField(info, bb);
            }
        }

        ///Used just after a textfield with no prefix to show an italic transparent text inside when empty
        public static void TextFieldComment(string check, string comment = "Comments...", float alpha = 0.5f) {
