...
namespace AIParadoxNotion.Design
{
	...
	partial class EditorUtils
    {
        ...
        public static GenericMenu GetTypeSelectionMenu(Type baseType, Action<Type> callback, GenericMenu menu = null, string subCategory = null, bool isSceneAI = false)
        {
            ...
            {
                if (baseType.BaseType == typeof(Task))
                {
                    if (isSceneAI && !info.isSceneNode)
                    {
                        continue;
                    }
                    else if (!isSceneAI && !info.isActorNode)
                    {
                        continue;
                    }
                }
