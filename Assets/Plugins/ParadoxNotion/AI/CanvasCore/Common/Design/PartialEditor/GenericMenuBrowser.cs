...
namespace AIParadoxNotion.Design
{
    ...
    public class GenericMenuBrowser : PopupWindowContent
    {
        ...
        //提示信息框高度
        private const float HELP_RECT_HEIGHT = 70;
        ...
        void DoTree(Rect treeRect, Event e) {
            ...
            foreach ( var childPair in currentNode.children ) {
                ...
                //Content
                var label = node.name;
                var text = string.Format("<size=12>{0}</size>", ( leafItem == null ? string.Format("<b>{0}</b>", label) : label ));
        ...
        void DoFooter(Rect helpRect, Event e) {
            ...
            GUILayout.Label(string.Format("<size=12>{0}</size>", doc), helpStyle);
            GUILayout.EndVertical();
            GUILayout.EndArea();
        }
