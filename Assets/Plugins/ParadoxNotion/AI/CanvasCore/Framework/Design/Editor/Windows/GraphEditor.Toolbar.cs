...
namespace AINodeCanvas.Editor
{
    ...
    public partial class GraphEditor
    {
        ...
        public static void ShowToolbar(Graph graph) {
            ...
            if ( GUILayout.Button(new GUIContent(CityNodeCanvas.Editor.StyleSheet.log, "Open Graph Console"), EditorStyles.toolbarButton, GUILayout.MaxHeight(12)) ) {
                GraphConsole.ShowWindow();
            }

            if ( GUILayout.Button(new GUIContent(CityNodeCanvas.Editor.StyleSheet.lens, "Open Graph Finder"), EditorStyles.toolbarButton, GUILayout.MaxHeight(12)) ) {
            ...
            GUILayout.Label(string.Format("{0} @NodeCanvas Framework v{1}", graph.GetType().Name, AINodeCanvas.Framework.Internal.GraphSerializationData.FRAMEWORK_VERSION), EditorStyles.toolbarButton);
            GUILayout.Space(10);
        ...
        static GenericMenu GetToolbarMenu_File(Graph graph, GraphOwner owner) {
            var menu = new GenericMenu();
            menu.AddItem(new GUIContent("清除所有节点"), false, () =>
           {
               if ( EditorUtility.DisplayDialog("警告", "当前操作会删除所有的行为树节点!\n确定继续执行?", "确定", "取消!") ) {
                   graph.ClearGraph();
               }
           });
        ...
        //   });
           ...
           {
               AIParadoxNotion.Serialization.JSONSerializer.ShowData(graph.Serialize(true, null), graph.name);
           });

            menu.AddItem(new GUIContent("导出数据"), false, () =>
           {
               int treeID = System.Convert.ToInt32(graph.name.Split('_')[0]);
               AIParadoxNotion.Serialization.JSONSerializer.SaveAITreeProto(graph.GetAITreeProto(treeID), treeID);
               //AIParadoxNotion.Serialization.JSONSerializer.SaveAITreeData(graph.GetAITreeData(true, null, treeID), graph.name, treeID);
           });
