...
using CanvasGroup = AINodeCanvas.Framework.CanvasGroup;
...
namespace AINodeCanvas.Editor
{
    ...
    public partial class GraphEditor : EditorWindow
    {
        ...
        void OnEnable() {
            current = this;
            titleContent = new GUIContent("Canvas", CityNodeCanvas.Editor.StyleSheet.canvasIcon);
        ...
        void OnGUI() {
            ...
            //canvas background
            GUI.Box(canvasRect, string.Empty, CityNodeCanvas.Editor.StyleSheet.canvasBG);
            //background grid
            ...
            if ( Application.isPlaying ) {
                ...
                GUI.Box(r, "PlayMode Active", CityNodeCanvas.Editor.StyleSheet.windowHighlight);
            }
            ...
            //closure
            GUI.Box(originalCanvasRect, string.Empty, CityNodeCanvas.Editor.StyleSheet.canvasBorders);
            GUI.color = Color.white;
            GUI.backgroundColor = Color.white;
        }
        ...
        static void ShowNodesGUI(Graph graph, Rect drawCanvas, bool fullDrawPass, Vector2 canvasMousePos, float zoomFactor) {
            ...
            if ( graph.primeNode != null ) {
                GUI.Box(new Rect(graph.primeNode.rect.x, graph.primeNode.rect.y - 20, graph.primeNode.rect.width, 20), "<b>START</b>", CityNodeCanvas.Editor.StyleSheet.box);
            }
        }
        ...
        static void DoCanvasRectSelection(Rect container) {
            ...
            if ( isMultiSelecting ) {
                ...
                if ( rect.width > 5 && rect.height > 5 ) {
                    ...
                    GUI.Box(rect, string.Empty, CityNodeCanvas.Editor.StyleSheet.box);
                    for ( var i = 0; i < currentGraph.allNodes.Count; i++ ) {
                        ...
                        if ( rect.Overlaps(node.rect) && !node.isHidden ) {
                            ...
                            GUI.Box(highlightRect, string.Empty, CityNodeCanvas.Editor.StyleSheet.windowHighlight);
                        }
                    }
                    if ( rect.width > 50 && rect.height > 50 ) {
                        GUI.color = new Color(1, 1, 1, e.control ? 0.6f : 0.15f);
                        GUI.Label(new Rect(e.mousePosition.x + 16, e.mousePosition.y, 120, 22), "<i>+ control for group</i>", CityNodeCanvas.Editor.StyleSheet.labelOnCanvas);
                    }
                }
            }
        ...
        static void DoBreadCrumbNavigation(Graph root) {
            ...
            if ( root.currentChildGraph == null ) {
                ...
                if ( root.agent == null && root.blackboard == null ) {
                    GUILayout.Label(string.Format("<b><size=22>{0} {1}</size></b>", root.name, graphInfo), CityNodeCanvas.Editor.StyleSheet.labelOnCanvas);
                } else {
                    ...
                    GUILayout.Label(string.Format("<b><size=22>{0} {1}</size></b>\n<size=12>{2} | {3}</size>", root.name, graphInfo, agentInfo, bbInfo), CityNodeCanvas.Editor.StyleSheet.labelOnCanvas);
                }

            } else {

                GUILayout.BeginHorizontal();
                GUILayout.Label("⤴ " + root.name, CityNodeCanvas.Editor.StyleSheet.button);
        ...
        static void DoCanvasGroups() {
            ...
            for ( var i = 0; i < currentGraph.canvasGroups.Count; i++ ) {
                ...
                var style = CityNodeCanvas.Editor.StyleSheet.editorPanel;
                ...
                GUI.color = Color.white;
                GUI.Box(new Rect(scaleRectBR.x + 10, scaleRectBR.y + 10, 6, 6), string.Empty, CityNodeCanvas.Editor.StyleSheet.scaleArrowBR);
        ...
        static void DrawMinimap(Rect container) {
            ...
            GUI.Box(container, string.Empty, CityNodeCanvas.Editor.StyleSheet.windowShadow);
            GUI.Box(container, currentGraph.allNodes.Count > 0 ? string.Empty : "Minimap", CityNodeCanvas.Editor.StyleSheet.box);
            ...
            if ( Prefs.minimapSize != Prefs.MINIMAP_MIN_SIZE ) {
                ...
                GUI.Box(lensRect, string.Empty, CityNodeCanvas.Editor.StyleSheet.box);
                GUI.color = Color.white;
                ...
                if ( currentGraph.allNodes != null ) {
                    ...
                    for ( var i = 0; i < currentGraph.allNodes.Count; i++ ) {
                        ...
                        for ( var j = 0; j < node.outConnections.Count; j++ ) {
                            ...
                            Handles.color = Application.isPlaying ? CityNodeCanvas.Editor.StyleSheet.GetStatusColor((CityNodeCanvas.Framework.Status)connection.status) : Colors.Grey(0.35f);
            ...
            var resizeRect = new Rect(container.x, container.y, 6, 6);
            GUI.color = Color.white;
            GUI.Box(resizeRect, string.Empty, CityNodeCanvas.Editor.StyleSheet.scaleArrowTL);
        }
        ...
        static void ShowConsoleLog() {
            ...
            if ( msg.IsValid() ) {
                rect.xMin += 2;
                GUI.Label(rect, GraphConsole.GetFormatedGUIContentForMessage(msg), CityNodeCanvas.Editor.StyleSheet.labelOnCanvas);
