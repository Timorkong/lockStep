...
namespace AINodeCanvas.Editor
{
    ...
    public class TaskEditor : EditorObjectWrapper<Task>
    {
        ...
        public static void ShowCreateTaskSelectionButton(ITaskSystem ownerSystem, Type baseType, Action<Task> callback) {
            ...
            var label = "添加 " + baseType.Name.SplitCamelCase();
            if ( GUILayout.Button(label) ) {
                ...
                bool isSceneAI = false;
                if (ownerSystem is BehaviourTree)
                {
                    isSceneAI = ((BehaviourTree)ownerSystem).isSceneAI;
                }

                var menu = EditorUtils.GetTypeSelectionMenu(baseType, TaskTypeSelected, null, null, isSceneAI);
        ...
        void ShowWarnings() {
            if ( task.firstWarningMessage != null ) {
                ...
                GUILayout.Label(string.Format("<size=12>{0}</size>", task.firstWarningMessage));
        ...
        void SpecialCaseInspector() {
            ...
            if ( task is ConditionTask ) {
                ...
                ( task as ConditionTask ).invert = EditorGUILayout.ToggleLeft("条件取反", ( task as ConditionTask ).invert);
                GUI.color = Color.white;
            }
        }
