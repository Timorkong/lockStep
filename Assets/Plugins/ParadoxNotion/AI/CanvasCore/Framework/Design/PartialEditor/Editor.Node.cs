...
namespace AINodeCanvas.Framework
{
    partial class Node
    {
        ...
        [HideInInspector]
        public EnumBTNodeType _NodeType;
        ...
        static void DrawNodeWindow(Node node, Vector2 canvasMousePos, float zoomFactor) {
            ...
            if ( node.collapsed ) {
                ...
                if ( GUI.Button(r, "单击展开", CityNodeCanvas.Editor.StyleSheet.box) ) {
            ...
            node.rect = GUILayout.Window(node.ID, node.rect, (ID) => { NodeWindowGUI(node, ID); }, string.Empty, CityNodeCanvas.Editor.StyleSheet.window, GUILayout.MaxHeight(MIN_SIZE.y), GUILayout.MaxWidth(MIN_SIZE.x));


            GUI.color = Color.white;
            GUI.Box(node.rect, string.Empty, CityNodeCanvas.Editor.StyleSheet.windowShadow);


            if ( Application.isPlaying && node.status != Status.Resting ) {
                GUI.color = CityNodeCanvas.Editor.StyleSheet.GetStatusColor((CityNodeCanvas.Framework.Status)node.status);
                GUI.Box(node.rect, string.Empty, CityNodeCanvas.Editor.StyleSheet.windowHighlight);

            } else {

                if ( node.isSelected ) {
                    GUI.color = CityNodeCanvas.Editor.StyleSheet.GetStatusColor((CityNodeCanvas.Framework.Status)Status.Resting);
                    GUI.Box(node.rect, string.Empty, CityNodeCanvas.Editor.StyleSheet.windowHighlight);
                }
            }
        ...
        static void ShowHeader(Node node) {
            ...
            if ( !node.showIcon || node.iconAlignment != Alignment2x2.Default ) {
                if ( node.name != null ) {
                    ...
                    if ( node.nodeColor != default(Color) ) {
                        ...
                        GUI.Box(new Rect(0, 0, node.rect.width, headerHeight), string.Empty, CityNodeCanvas.Editor.StyleSheet.windowHeader);
                        GUI.color = Color.white;
                    }
                    ...
                    GUILayout.Label(content, CityNodeCanvas.Editor.StyleSheet.windowTitle, GUILayout.MaxHeight(23));
                }
            }
            ...
            if ( node.showIcon && ( node.iconAlignment == Alignment2x2.Default || node.iconAlignment == Alignment2x2.Bottom ) ) {
                ...
                GUILayout.Box(node.icon, CityNodeCanvas.Editor.StyleSheet.box, GUILayout.MaxHeight(32));
        ...
        static void ShowStatusIcons(Node node) {
            if ( Application.isPlaying && node.status != Status.Resting ) {
                ...
                if ( node.status == Status.Success ) {
                    GUI.color = CityNodeCanvas.Editor.StyleSheet.GetStatusColor((CityNodeCanvas.Framework.Status)Status.Success);
                    GUI.DrawTexture(markRect, CityNodeCanvas.Editor.StyleSheet.statusSuccess);

                } else if ( node.status == Status.Running ) {
                    GUI.color = CityNodeCanvas.Editor.StyleSheet.GetStatusColor((CityNodeCanvas.Framework.Status)Status.Running);
                    GUI.DrawTexture(markRect, CityNodeCanvas.Editor.StyleSheet.statusRunning);

                } else if ( node.status == Status.Failure ) {
                    GUI.color = CityNodeCanvas.Editor.StyleSheet.GetStatusColor((CityNodeCanvas.Framework.Status)Status.Failure);
                    GUI.DrawTexture(markRect, CityNodeCanvas.Editor.StyleSheet.statusFailure);
                }
            }
        }
        ...
        static GenericMenu GetNodeMenu_Single(Node node) {
            ...
            if ( node.graph.primeNode != node && node.allowAsPrime ) {
                ...
                menu.AddItem(new GUIContent("设置为开始节点"), false, () => { node.graph.primeNode = node; });
            }
            ...
            menu.AddItem(new GUIContent("复制节点"), false, () => { CopyBuffer.Set<Node[]>(new Node[] { node }); });
            ...
            if ( node.graph.isTree && node.outConnections.Count > 0 ) {
                menu.AddItem(new GUIContent(node.collapsed ? "展开子节点" : "收起子节点"), false, () => { node.collapsed = !node.collapsed; });
            }
            ...
            if ( node is ITaskAssignable ) {
                ...
                if ( CopyBuffer.Has<Task>() ) {
                    menu.AddItem(new GUIContent("Paste Assigned Task"), false, () =>
                   {
                       ...
                       catch { AIParadoxNotion.Services.Logger.LogWarning("Can't paste Task here. Incombatible Types", "Editor", node); }
                   });
        ...
        static void DrawNodeComments(Node node) {
            ...
            var style = CityNodeCanvas.Editor.StyleSheet.commentsBox;
            ...
            GUI.Box(commentsRect, node.comments, CityNodeCanvas.Editor.StyleSheet.commentsBox);
            GUI.backgroundColor = Color.white;
            GUI.color = Color.white;
        }
        ...
        static void DrawNodeTag(Node node) {
            if ( !string.IsNullOrEmpty(node.tag) ) {
                var size = CityNodeCanvas.Editor.StyleSheet.labelOnCanvas.CalcSize(new GUIContent(node.tag));
                var tagRect = new Rect(node.rect.x - size.x - 10, node.rect.y, size.x, size.y);
                GUI.Label(tagRect, node.tag, CityNodeCanvas.Editor.StyleSheet.labelOnCanvas);
        ...
        static void DrawNodeID(Node node) {
            if ( Prefs.showNodeIDs ) {
                ...
                GUI.Label(rect, string.Format("<size=12>#{0}</size>", node.ID.ToString()), CityNodeCanvas.Editor.StyleSheet.labelOnCanvas);
                GUI.color = Color.white;
            }
        }
        ...
        static public void ShowNodeInspectorGUI(Node node) {
            ...
            if ( Prefs.showNodeInfo ) {
                ...
                //and by kevinwu
                GUILayout.Space(8);
                EditorUtils.LabelField(node.description, 11, true);
                 
                // EditorGUILayout.HelpBox(node.description, MessageType.Info);
                GUI.backgroundColor = Color.white;
            ...
            //     EditorUtils.TextFieldComment(node.customName, "名称...");
            // }

            node.tag = EditorGUILayout.TextField(node.tag);
            // EditorUtils.TextFieldComment(node.tag, "Tag...");
            EditorUtils.TextFieldComment(node.tag, "标签...");
            ...
            EditorUtils.TextFieldComment(node.comments, "描述...");
        ...
        virtual protected void DrawNodeConnections(Rect drawCanvas, bool fullDrawPass, Vector2 canvasMousePos, float zoomFactor) {
            ...
            if ( fullDrawPass || drawCanvas.Overlaps(rect) ) {
                ...
                GUI.Box(nodeOutputBox, string.Empty, CityNodeCanvas.Editor.StyleSheet.nodePortContainer);
                ...
                if ( outConnections.Count < maxOutConnections || maxOutConnections == -1 ) {
                    for ( var i = 0; i < outConnections.Count + 1; i++ ) {
                        ...
                        GUI.Box(portRect, string.Empty, CityNodeCanvas.Editor.StyleSheet.nodePortEmpty);
            ...
            if ( clickedPort != null && clickedPort.parent == this ) {
                ...
                AIParadoxNotion.CurveUtils.ResolveTangents(clickedPort.pos, e.mousePosition, 0.5f, PlanarDirection.Vertical, out tangA, out tangB);
                Handles.DrawBezier(clickedPort.pos, e.mousePosition, clickedPort.pos + tangA, e.mousePosition + tangB, CityNodeCanvas.Editor.StyleSheet.GetStatusColor((CityNodeCanvas.Framework.Status)Status.Resting).WithAlpha(0.8f), null, 3);
            }
            ...
            for ( var i = 0; i < outConnections.Count; i++ ) {
                ...
                if ( connection != null ) {
                    ...
                    if ( fullDrawPass || drawCanvas.Overlaps(boundRect) ) {
                        ...
                        GUI.Box(sourcePortRect, string.Empty, CityNodeCanvas.Editor.StyleSheet.nodePortConnected);
