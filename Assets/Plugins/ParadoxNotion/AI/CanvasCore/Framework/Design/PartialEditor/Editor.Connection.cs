...
namespace AINodeCanvas.Framework
{
    ...
    partial class Connection
    {
        ...
        private Color color = CityNodeCanvas.Editor.StyleSheet.GetStatusColor(CityNodeCanvas.Framework.Status.Resting);
        ...
        virtual public Color defaultColor {
            get { return CityNodeCanvas.Editor.StyleSheet.GetStatusColor((CityNodeCanvas.Framework.Status)status); }
        }
        ...
        virtual public AIParadoxNotion.PlanarDirection direction {
            get { return AIParadoxNotion.PlanarDirection.Auto; }
        }
        ...
        public void DrawConnectionGUI(Vector2 fromPos, Vector2 toPos) {
            ...
            AIParadoxNotion.CurveUtils.ResolveTangents(fromPos, toPos, sourceNode.rect, targetNode.rect, 0.8f, direction, out fromTangent, out toTangent);
        ...
        void HandleEvents(Vector2 fromPos, Vector2 toPos) {
            ...
            if ( GraphEditorUtility.allowClick && e.type == EventType.MouseDown ) {
                float norm;
                var onConnection = AIParadoxNotion.CurveUtils.IsPosAlongCurve(fromPos, toPos, fromTangent, toTangent, e.mousePosition, out norm);
        ...
        void DrawConnection(Vector2 fromPos, Vector2 toPos) {
            ...
            if ( isRelinkingActive ) {
                ...
                AIParadoxNotion.CurveUtils.ResolveTangents(fromPos, toPos, 0.8f, direction, out fromTangent, out toTangent);
                size = defaultSize;
            }
            ...
            if ( tipConnectionStyle == TipConnectionStyle.Arrow ) {
                GUI.DrawTexture(endRect, CityNodeCanvas.Editor.StyleSheet.GetDirectionArrow(toTangent.normalized));
            }
            if ( tipConnectionStyle == TipConnectionStyle.Circle ) {
                GUI.DrawTexture(endRect, CityNodeCanvas.Editor.StyleSheet.circle);
            }
        ...
        void DrawInfoRect(Vector2 fromPos, Vector2 toPos) {
            ...
            centerRect.center = AIParadoxNotion.CurveUtils.GetPosAlongCurve(fromPos, toPos, fromTangent, toTangent, 0.55f);
            ...
            if ( !string.IsNullOrEmpty(info) || !string.IsNullOrEmpty(extraInfo) ) {
                ...
                var textToShow = isExpanded ? string.Format("<size=12>{0}{1}</size>", info, extraInfo) : "<size=10>...</size>";
                var finalSize = CityNodeCanvas.Editor.StyleSheet.box.CalcSize(new GUIContent(textToShow));
        ...
        void UpdateBlinkStatus(Vector2 fromPos, Vector2 toPos) {
            ...
            if ( status == Status.Running ) {
                ...
                for ( var i = 0f; i < STATUS_BLINK_PACKET_COUNT; i++ ) {
                    ...
                    rect.center = AIParadoxNotion.CurveUtils.GetPosAlongCurve(fromPos, toPos, fromTangent, toTangent, normPos); ;
                    GUI.color = packetColor;
                    GUI.DrawTexture(rect, CityNodeCanvas.Editor.StyleSheet.circle);
                    GUI.color = Color.white;
                }
            }
        }
