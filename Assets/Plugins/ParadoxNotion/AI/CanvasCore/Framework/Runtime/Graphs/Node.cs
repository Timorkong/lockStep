...
namespace AINodeCanvas.Framework
{
    ...
    [System.Serializable]
    [AIParadoxNotion.Design.SpoofAOT]
    abstract public partial class Node : IGraphElement
    {
        ...
        public static Node Create(Graph targetGraph, System.Type nodeType, Vector2 pos) {
            ...
            if ( targetGraph == null ) {
                AIParadoxNotion.Services.Logger.LogError("Can't Create a Node without providing a Target Graph", "NodeCanvas");
                return null;
            }
        ...
        public Node Duplicate(Graph targetGraph) {
            ...
            if ( targetGraph == null ) {
                AIParadoxNotion.Services.Logger.LogError("Can't duplicate a Node without providing a Target Graph", "NodeCanvas");
                return null;
            }
        ...
        public Status Execute(Component agent, IBlackboard blackboard) {
            ...
            if ( isBreakpoint ) {
                if ( status == Status.Resting ) {
                    var breakEditor = AINodeCanvas.Editor.Prefs.breakpointPauseEditor;
                    var owner = agent as GraphOwner;
                    var contextName = owner != null ? owner.gameObject.name : graph.name;
                    AIParadoxNotion.Services.Logger.LogWarning(string.Format("Node: '{0}' | ID: '{1}' | Graph Type: '{2}' | Context Object: '{3}'", name, ID, graph.GetType().Name, contextName), "Breakpoint", this);
        ...
        public Status Error(object msg) {
            if ( msg is System.Exception ) {
                AIParadoxNotion.Services.Logger.LogException((System.Exception)msg, "Execution", this);
            } else {
                AIParadoxNotion.Services.Logger.LogError(msg, "Execution", this);
            }
            status = Status.Error;
            return Status.Error;
        }
        ...
        public void Warn(string msg) {
            AIParadoxNotion.Services.Logger.LogWarning(msg, "Execution", this);
        }
        ...
        public void RegisterEvents(Component targetAgent, params string[] eventNames) {
            if ( targetAgent == null ) {
                AIParadoxNotion.Services.Logger.LogError("Null Agent provided for event registration", "Events", this);
                return;
            }
        ...
        public static bool IsNewConnectionAllowed(Node sourceNode, Node targetNode, Connection refConnection = null) {
            ...
            if ( sourceNode == null || targetNode == null ) {
                AIParadoxNotion.Services.Logger.LogWarning("A Node Provided is null.", "Editor", targetNode);
                return false;
            }

            if ( sourceNode == targetNode ) {
                AIParadoxNotion.Services.Logger.LogWarning("Node can't connect to itself.", "Editor", targetNode);
                return false;
            }
            ...
            if ( refConnection == null || refConnection.sourceNode != sourceNode ) {
                if ( sourceNode.outConnections.Count >= sourceNode.maxOutConnections && sourceNode.maxOutConnections != -1 ) {
                    AIParadoxNotion.Services.Logger.LogWarning("Source node can have no more out connections.", "Editor", sourceNode);
                    return false;
                }
            }
            ...
            if ( refConnection == null || refConnection.targetNode != targetNode ) {
                if ( targetNode.maxInConnections <= targetNode.inConnections.Count && targetNode.maxInConnections != -1 ) {
                    AIParadoxNotion.Services.Logger.LogWarning("Target node can have no more in connections.", "Editor", targetNode);
                    return false;
                }
            }
