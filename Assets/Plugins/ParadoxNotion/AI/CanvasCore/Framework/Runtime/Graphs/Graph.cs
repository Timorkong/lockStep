...
using AIParadoxNotion.Serialization;
using AIParadoxNotion.Serialization.FullSerializer;
...
namespace AINodeCanvas.Framework
{
    ...
    abstract public partial class Graph : ScriptableObject, ITaskSystem, ISerializationCallbackReceiver
    {
        ...
        //封装行为树数据
        public string GetAITreeData(bool pretyJson, List<UnityEngine.Object> objectReferences, int treeID)
        {
            if ( _deserializationFailed ) {
                Logger.Log("Due to last deserialization attempt failure, this graph is protected from changes.\nAny change you make is not saved until the graph has deserialized successfully.\nPlease try restarting Unity to attempt deserialization again.\nIf you think this is a bug, please contact support.", "Editor", this);
                return _serializedGraph;
            ...
            UpdateNodeIDs(true);
            // return JSONSerializer.Serialize(typeof(AITreeConfig), new AITreeConfig(this.allNodes), pretyJson, objectReferences);
            // string json = UnityEngine.JsonUtility.ToJson(new AITreeConfig(this.allNodes), true);
            string json = AITreeConfig.GetAITreeConfig(this.allNodes, treeID, this.blackboard);
            //PBTools.WriteToFile(roomData, PBTools.CallFileWindow(true));
            // JavaScriptSerializer.
            UnityEngine.Debug.Log("node count = " + this.allNodes.Count);
            return json;
            //导出行为树json数据
            // return JSONSerializer.Serialize(typeof(GraphSerializationData), (new GraphSerializationData(this)).GetAITreeJsonData(), pretyJson, objectReferences);
        }

        public Pbe.AITree GetAITreeProto(int treeID)
        {
            UpdateNodeIDs(true);

            if (blackboard != null)
            {
                Pbe.AITree protoTree = AITreeConfig.GetAITreeProto(this.allNodes, treeID, blackboard);
                Debug.Log("node count = " + this.allNodes.Count);
                return protoTree;
            }
            else
            {
                var bp = GetDefinedParameters();
                Pbe.AITree protoTree = AITreeConfig.GetAITreeProto(this.allNodes, treeID, bp);
                Debug.Log("node count = " + this.allNodes.Count);
                return protoTree;
            }
        }
        ...
        public void RemoveNode(Node node, bool recordUndo = true, bool force = false) {
            ...
            if ( !force && node.GetType().RTIsDefined<AIParadoxNotion.Design.ProtectedSingletonAttribute>(true) ) {
            ...
            if ( AINodeCanvas.Editor.GraphEditorUtility.activeElement == node ) {
                AINodeCanvas.Editor.GraphEditorUtility.activeElement = null;
            }
#endif
...
#if UNITY_EDITOR
            if ( AINodeCanvas.Editor.GraphEditorUtility.activeElement == connection ) {
                AINodeCanvas.Editor.GraphEditorUtility.activeElement = null;
            }
#endif
