...
namespace AINodeCanvas.Framework.Internal
{
    ...
    sealed public class BlackboardSource : IBlackboard
    {
        ...
        public T GetValue<T>(string varName) {
            ...
            catch { Debug.LogError(string.Format("<b>Blackboard:</b> Can't cast value of variable with name '{0}' to type '{1}'", varName, typeof(T).FriendlyName())); }
            return default(T);
        }
        ...
        public Variable SetValue(string varName, object value) {
            ...
            try { variable.value = value; }
                ...
                Debug.LogError(string.Format("<b>Blackboard:</b> Can't cast value '{0}' to blackboard variable of name '{1}' and type '{2}'", value != null ? value.ToString() : "null", varName, variable.varType.FriendlyName()));
                return null;
            }
