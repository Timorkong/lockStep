    ...
    abstract public partial class Task : ISubParametersContainer
    {
        ...
        public void SetOwnerSystem(ITaskSystem newOwnerSystem) {
            ...
            if ( newOwnerSystem == null ) {
                AIParadoxNotion.Services.Logger.LogError("ITaskSystem set in task is null!!", "Init", this);
                return;
            }
...
#if UNITY_EDITOR
                {
                    if ( !AINodeCanvas.Editor.Prefs.showTaskSummary ) {
        ...
        protected bool Error(string error) {
            AIParadoxNotion.Services.Logger.LogError(string.Format("{0} | TASK DISABLED | {1}", error, ownerSystem != null ? ownerSystem.contextObject : null), "Task", this);
            return false;
        }
