...
namespace AINodeCanvas.BehaviourTrees
{
    ...
    // [Category("组合节点")]
    [Category("暂不开放")]
    [Name("优先选择器", EnumCompositesPriority.PrioritySelector)]
    [Description("优先级选择器执行优先级权重最高的子级。如果失败，优先级选择器将继续处理下一个优先级最高的子级，直到一个成功或全部失败为止（类似于正常情况选择器）")]
    ...
    public class PrioritySelector : BTComposite
    {
        ...
        public override void OnConnectionInspectorGUI(int i) {
            priorities[i] = (BBParameter<float>)AINodeCanvas.Editor.BBParameterEditor.ParameterField("Priority Weight", priorities[i]);
        }
