...
namespace AINodeCanvas.StateMachines
{
        ...
        sealed protected override void DrawNodeConnections(Rect drawCanvas, bool fullDrawPass, Vector2 canvasMousePos, float zoomFactor) {
            ...
            if ( maxOutConnections != 0 ) {
                if ( fullDrawPass || drawCanvas.Overlaps(rect) ) {
                    ...
                    GUI.DrawTexture(portRectLeft, CityNodeCanvas.Editor.StyleSheet.arrowLeft);
                    GUI.DrawTexture(portRectRight, CityNodeCanvas.Editor.StyleSheet.arrowRight);
                    if ( maxInConnections == 0 ) {
                        GUI.DrawTexture(portRectBottom, CityNodeCanvas.Editor.StyleSheet.arrowBottom);
                    }
