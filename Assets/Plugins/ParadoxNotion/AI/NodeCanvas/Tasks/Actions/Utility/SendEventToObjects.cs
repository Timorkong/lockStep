        ...
        protected override void OnExecute() {
            ...
            foreach ( var target in targetObjects.value ) {
                if ( target != null ) {
                    ...
                    if ( owner != null ) {
                        owner.SendEvent(new AIParadoxNotion.EventData(eventName.value), this);
                    }
                }
            }
    ...
    public class SendEventToObjects<T> : ActionTask
    {
        ...
        protected override void OnExecute() {
            ...
            foreach ( var target in targetObjects.value ) {
                var owner = target.GetComponent<GraphOwner>();
                if ( owner != null ) {
                    owner.SendEvent(new AIParadoxNotion.EventData<T>(eventName.value, eventValue.value), this);
                }
            }
