            ...
            if ( m != null ) {
                ...
                if ( m.ReturnType == typeof(void) ) {
                    for ( var i = 0; i < paramNames.Length; i++ ) {
                        AINodeCanvas.Editor.BBParameterEditor.ParameterField(paramNames[i], variables[i]);
                    }
                } else {
                    for ( var i = 0; i < paramNames.Length; i++ ) {
                        AINodeCanvas.Editor.BBParameterEditor.ParameterField(paramNames[i], variables[i + 1]);
                    }

                    if ( m.ReturnType != typeof(IEnumerator) ) {
                        AINodeCanvas.Editor.BBParameterEditor.ParameterField("Save Return Value", variables[0], true);
                    }
                }
            }
        }
