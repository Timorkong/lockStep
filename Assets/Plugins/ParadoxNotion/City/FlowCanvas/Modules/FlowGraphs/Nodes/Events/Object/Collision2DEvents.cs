...
namespace FlowCanvas.Nodes{
	...
	public class Collision2DEvents : MessageEventNode<Collider2D> {
		...
		void OnCollisionEnter2D(CityParadoxNotion.Services.MessageRouter.MessageData<Collision2D> msg){
			this.receiver = ResolveReceiver(msg.receiver);
			this.collision = msg.value;
			onEnter.Call(new Flow());
		}

		void OnCollisionStay2D(CityParadoxNotion.Services.MessageRouter.MessageData<Collision2D> msg){
			this.receiver = ResolveReceiver(msg.receiver);
			this.collision = msg.value;
			onStay.Call(new Flow());
		}

		void OnCollisionExit2D(CityParadoxNotion.Services.MessageRouter.MessageData<Collision2D> msg){
			this.receiver = ResolveReceiver(msg.receiver);
			this.collision = msg.value;
