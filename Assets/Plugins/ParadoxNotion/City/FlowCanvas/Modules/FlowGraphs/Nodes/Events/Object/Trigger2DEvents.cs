...
namespace FlowCanvas.Nodes{
	...
	public class Trigger2DEvents : MessageEventNode<Collider2D> {
		...
		void OnTriggerEnter2D(CityParadoxNotion.Services.MessageRouter.MessageData<Collider2D> msg){
			this.receiver = ResolveReceiver(msg.receiver);
			this.other = msg.value.gameObject;
			onEnter.Call(new Flow());
		}

		void OnTriggerStay2D(CityParadoxNotion.Services.MessageRouter.MessageData<Collider2D> msg){
			this.receiver = ResolveReceiver(msg.receiver);
			this.other = msg.value.gameObject;
			onStay.Call(new Flow());
		}

		void OnTriggerExit2D(CityParadoxNotion.Services.MessageRouter.MessageData<Collider2D> msg){
			this.receiver = ResolveReceiver(msg.receiver);
			this.other = msg.value.gameObject;
