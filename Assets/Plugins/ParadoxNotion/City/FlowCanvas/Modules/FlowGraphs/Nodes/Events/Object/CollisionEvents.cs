...
namespace FlowCanvas.Nodes{
	...
	public class CollisionEvents : MessageEventNode<Collider> {
		...
		void OnCollisionEnter(CityParadoxNotion.Services.MessageRouter.MessageData<Collision> msg){
			this.receiver = ResolveReceiver(msg.receiver);
			this.collision = msg.value;
			onEnter.Call(new Flow());
		}

		void OnCollisionStay(CityParadoxNotion.Services.MessageRouter.MessageData<Collision> msg){
			this.receiver = ResolveReceiver(msg.receiver);
			this.collision = msg.value;
			onStay.Call(new Flow());
		}

		void OnCollisionExit(CityParadoxNotion.Services.MessageRouter.MessageData<Collision> msg){
			this.receiver = ResolveReceiver(msg.receiver);
			this.collision = msg.value;
