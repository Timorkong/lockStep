...
namespace FlowCanvas.Nodes{
	...
	public class ObjectStateEvents : MessageEventNode<Transform> {
		...
		void OnEnable(CityParadoxNotion.Services.MessageRouter.MessageData msg){
			this.receiver = ResolveReceiver(msg.receiver).gameObject;
			onEnable.Call(new Flow());
		}

		void OnDisable(CityParadoxNotion.Services.MessageRouter.MessageData msg){
			this.receiver = ResolveReceiver(msg.receiver).gameObject;
			onDisable.Call(new Flow());
		}

		void OnDestroy(CityParadoxNotion.Services.MessageRouter.MessageData msg){
			this.receiver = ResolveReceiver(msg.receiver).gameObject;
