	...
	abstract public class LatentActionNodeBase : SimplexNode {
		...
		protected void Break(){
			if (currentCoroutine != null){
				...
				done.parent.SetStatus(CityNodeCanvas.Framework.Status.Resting);
				OnBreak();
				if (!graphStoped){
					done.Call(new Flow());
				}
		...
		IEnumerator InternalCoroutine(IEnumerator enumerator, Flow f){
			parentNode.SetStatus(CityNodeCanvas.Framework.Status.Running);
			if (outFlow != null){
				outFlow.Call(f);
			}
			...
			parentNode.SetStatus(CityNodeCanvas.Framework.Status.Resting);
			done.Call(f);
			currentCoroutine = null;
