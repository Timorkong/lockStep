...
namespace FlowCanvas
{
	...
	abstract public class Port{
		...
		internal void EnsureCachedGUIContent(){
			...
			if (IsFlowPort()){
				...
				var hexColor = ColorUtils.ColorToHex( CityParadoxNotion.Design.TypePrefs.GetTypeColor(type) );
				...
				Texture icon = null;
				if (CityNodeCanvas.Editor.Prefs.showIcons){
					...
					icon = CityParadoxNotion.Design.TypePrefs.GetTypeIcon( isWildDef? typeof(Wild) : type );
				}
	...
	public class FlowOutput : Port{
		...
		public void Call(Flow f){
			if (pointer != null && !parent.graph.isPaused){
				...
				if (parent.isBreakpoint){
					CityParadoxNotion.Services.MonoManager.current.StartCoroutine(DebugBreakWait(f));
					return;
				}
		...
		System.Collections.IEnumerator DebugBreakWait(Flow f){
			CityParadoxNotion.Services.Logger.LogWarning(string.Format("On Node '{0}'", parent.name), "Breakpoint", parent);
			UnityEngine.Debug.Break();
			parent.SetStatus(CityNodeCanvas.Framework.Status.Running);
			yield return null;
			parent.SetStatus(CityNodeCanvas.Framework.Status.Resting);
			Continue(f);
		}
