...
namespace FlowCanvas {
	...
	abstract public class FlowGraph : Graph {
		...
		protected override void OnGraphEditorToolbar(){
			if (GUILayout.Button(new GUIContent(CityNodeCanvas.Editor.StyleSheet.verboseLevel1, "Minimize All/Selected"), UnityEditor.EditorStyles.toolbarButton, GUILayout.MaxHeight(12))){
				foreach(var node in CityNodeCanvas.Editor.GraphEditorUtility.GetSelectedOrAll(this).OfType<FlowNode>()){
					node.verboseLevel = Node.VerboseLevel.Compact;
				}
			}
			if (GUILayout.Button(new GUIContent(CityNodeCanvas.Editor.StyleSheet.verboseLevel2, "Show Connected Ports Only On All/Selected"), UnityEditor.EditorStyles.toolbarButton, GUILayout.MaxHeight(12))){
				foreach(var node in CityNodeCanvas.Editor.GraphEditorUtility.GetSelectedOrAll(this).OfType<FlowNode>()){
					node.verboseLevel = Node.VerboseLevel.Partial;
				}
			}
			if (GUILayout.Button(new GUIContent(CityNodeCanvas.Editor.StyleSheet.verboseLevel3, "Maximize All/Selected\nHold S Key over a node to temporarily maximize it"), UnityEditor.EditorStyles.toolbarButton, GUILayout.MaxHeight(12))){
				foreach(var node in CityNodeCanvas.Editor.GraphEditorUtility.GetSelectedOrAll(this).OfType<FlowNode>()){
		...
		protected override void OnDropAccepted(Object o, Vector2 mousePos){
			...
			CityParadoxNotion.Design.EditorUtils.ShowAsBrowser(menu, "Add Node For Drag & Drop Instance");
			Event.current.Use();
		}
