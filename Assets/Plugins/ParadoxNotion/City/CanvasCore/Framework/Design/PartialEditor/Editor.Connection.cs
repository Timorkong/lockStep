...
namespace CityNodeCanvas.Framework {
    ...
    partial class Connection{
		...
		virtual public CityParadoxNotion.PlanarDirection direction{
			get {return CityParadoxNotion.PlanarDirection.Auto;}
		}
		...
		public void DrawConnectionGUI(Vector2 fromPos, Vector2 toPos){
			...
			CityParadoxNotion.CurveUtils.ResolveTangents(fromPos, toPos, sourceNode.rect, targetNode.rect, 0.8f, direction, out fromTangent, out toTangent);
		...
		void HandleEvents(Vector2 fromPos, Vector2 toPos){
			...
			if ( GraphEditorUtility.allowClick && e.type == EventType.MouseDown){
				float norm;
				var onConnection = CityParadoxNotion.CurveUtils.IsPosAlongCurve(fromPos, toPos, fromTangent, toTangent, e.mousePosition, out norm);
		...
		void DrawConnection(Vector2 fromPos, Vector2 toPos){
			...
			if (isRelinkingActive){
				...
				CityParadoxNotion.CurveUtils.ResolveTangents(fromPos, toPos, 0.8f, direction, out fromTangent, out toTangent);
				size = defaultSize;
			}
		...
		void DrawInfoRect(Vector2 fromPos, Vector2 toPos){
			...
			centerRect.center = CityParadoxNotion.CurveUtils.GetPosAlongCurve(fromPos, toPos, fromTangent, toTangent, 0.55f);
		...
		void UpdateBlinkStatus(Vector2 fromPos, Vector2 toPos){
			...
			if (status == Status.Running){
				...
				for (var i = 0f; i < STATUS_BLINK_PACKET_COUNT; i++){
					...
					rect.center = CityParadoxNotion.CurveUtils.GetPosAlongCurve(fromPos, toPos, fromTangent, toTangent, normPos);;
		...
		public static void ShowConnectionInspectorGUI(Connection c){
			...
			c.type =(int)(ConnectionType)EditorGUILayout.EnumPopup( (ConnectionType)c.type);
