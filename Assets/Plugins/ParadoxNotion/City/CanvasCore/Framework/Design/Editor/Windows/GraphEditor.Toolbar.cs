...
using static CityNodeCanvas.Editor.CityLinkConfig;
using LitJson;
...
namespace CityNodeCanvas.Editor {
    ...
    public partial class GraphEditor {
		...
		public static void ShowToolbar(Graph graph){
			...
			GUILayout.Label(string.Format("{0} @NodeCanvas Framework v{1}", graph.GetType().Name, CityNodeCanvas.Framework.Internal.GraphSerializationData.FRAMEWORK_VERSION), EditorStyles.toolbarButton);
			GUILayout.Space(10);
		...
		static GenericMenu GetToolbarMenu_File(Graph graph, GraphOwner owner){
			...
			{
				CityParadoxNotion.Serialization.JSONSerializer.ShowData( graph.Serialize(true, null), graph.name );
			});

			menu.AddItem(new GUIContent("导出到游戏"), false, () =>
			{
				//ParadoxNotion.Serialization.JSONSerializer.ShowData(graph.Serialize(true, null), graph.name);

				//graph.Serialize(true, null)
				string json = graph.Serialize(true, null);

				JsonData data = JsonMapper.ToObject(json);
				JsonData nodeList = data["nodes"];
				JsonData connectionList = data["connections"];
				MapLinkConfig mapConfig = new MapLinkConfig();
				foreach (JsonData nodeinfo in nodeList)
				{
					MapLinkNode node = JsonUtility.FromJson<MapLinkNode>(nodeinfo.ToJson());
					node.uid = nodeinfo["$id"].ToString();
					node.RoomType = GetRoomType((string)nodeinfo["$type"]);
					if (node.NodeList.Contains(0))
					{
						EditorUtility.DisplayDialog("提示", "城市节点未配置", "确认", string.Empty);
						return;
					}
					if (node.CityID == 0 && node.RoomType!= (int)CityNodeType.ModuleNode)
					{
						EditorUtility.DisplayDialog("提示", "城市ID未配置", "确认", string.Empty);
						return;
					}
					mapConfig.nodes.Add(node);
				}
				foreach (JsonData nodeinfo in connectionList)
				{
					JsonData srcNode = nodeinfo["_sourceNode"]["$ref"];
					JsonData targetNode = nodeinfo["_targetNode"]["$ref"];
					MapConnectionNode node = JsonUtility.FromJson<MapConnectionNode>(nodeinfo.ToJson());
					node.srcNode = srcNode.ToString();
					node.targetNode = targetNode.ToString();
					mapConfig.connections.Add(node);
				}
				string exportJson = JsonMapper.ToJson(mapConfig);
				string clientPath = "../laya/bin/res/config/citylinkconfig.json";
				System.IO.File.WriteAllText(clientPath, exportJson);

        
                string configPath = "../../config/Config/ServerEditorConfig/citylinkconfig.json";
                System.IO.File.WriteAllText(configPath, exportJson);

                EditorUtility.DisplayDialog("提示", "导出成功", "确认", string.Empty);
			});
