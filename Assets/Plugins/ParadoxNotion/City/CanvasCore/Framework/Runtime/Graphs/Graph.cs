...
namespace CityNodeCanvas.Framework{
	...
	abstract public partial class Graph : ScriptableObject, ITaskSystem, ISerializationCallbackReceiver {
		...
		public void RemoveNode(Node node, bool recordUndo = true, bool force = false){
			...
			if (!force && node.GetType().RTIsDefined<CityParadoxNotion.Design.ProtectedSingletonAttribute>(true)){
			...
			if (CityNodeCanvas.Editor.GraphEditorUtility.activeElement == node){
				CityNodeCanvas.Editor.GraphEditorUtility.activeElement = null;
			}
			#endif
		...
		public void RemoveConnection(Connection connection, bool recordUndo = true){
			...
			#if UNITY_EDITOR
			if (CityNodeCanvas.Editor.GraphEditorUtility.activeElement == connection){
				CityNodeCanvas.Editor.GraphEditorUtility.activeElement = null;
			}
			#endif
