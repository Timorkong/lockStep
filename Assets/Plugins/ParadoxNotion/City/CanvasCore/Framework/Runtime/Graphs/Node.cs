...
using System.Linq;
using CityNodeCanvas.Framework.Internal;
...
using CityParadoxNotion.Services;
using UnityEngine;


namespace CityNodeCanvas.Framework{
    ...
    [System.Serializable]
    [CityParadoxNotion.Design.SpoofAOT]
	abstract public partial class Node : IGraphElement{
		...
		public static Node Create(Graph targetGraph, System.Type nodeType, Vector2 pos){
			...
			if (targetGraph == null){
                CityParadoxNotion.Services.Logger.LogError("Can't Create a Node without providing a Target Graph", "NodeCanvas");
				return null;
			}
		...
		public Node Duplicate(Graph targetGraph){
			...
			if (targetGraph == null){
                CityParadoxNotion.Services.Logger.LogError("Can't duplicate a Node without providing a Target Graph", "NodeCanvas");
				return null;
			}
		...
		public Status Execute(Component agent, IBlackboard blackboard){
			...
			if (isBreakpoint && status == Status.Resting){
				var breakEditor = CityNodeCanvas.Editor.Prefs.breakpointPauseEditor;
				var owner = agent as GraphOwner;
				var contextName = owner != null? owner.gameObject.name : graph.name;
                CityParadoxNotion.Services.Logger.LogWarning(string.Format("Node: '{0}' | ID: '{1}' | Graph Type: '{2}' | Context Object: '{3}'", name, ID, graph.GetType().Name, contextName), "Breakpoint", this);
				if (owner != null){
		...
		protected Status Error(string msg){
            CityParadoxNotion.Services.Logger.LogError(string.Format("{0} | On Node '{1}' | ID '{2}' | Graph '{3}'", msg, name, ID, graph.name), "Execution", this);
			status = Status.Error;
			return Status.Error;
		}
		...
		protected Status Error(System.Exception e){
            CityParadoxNotion.Services.Logger.LogException(e, "Execution", this);
			status = Status.Error;
			return Status.Error;
		}
		...
		public void RegisterEvents(Component targetAgent, params string[] eventNames){
			if (targetAgent == null){
                CityParadoxNotion.Services.Logger.LogError("Null Agent provided for event registration", "Events", this);
				return;
			}
		...
		public static bool IsNewConnectionAllowed(Node sourceNode, Node targetNode, Connection refConnection = null){
			...
			if (sourceNode == null || targetNode == null){
				CityParadoxNotion.Services.Logger.LogWarning("A Node Provided is null.", "Editor", targetNode);
				return false;
			}

			if (sourceNode == targetNode){
				CityParadoxNotion.Services.Logger.LogWarning("Node can't connect to itself.", "Editor", targetNode);
				return false;
			}
			...
			if (refConnection == null || refConnection.sourceNode != sourceNode){
				if (sourceNode.outConnections.Count >= sourceNode.maxOutConnections && sourceNode.maxOutConnections != -1){
                    CityParadoxNotion.Services.Logger.LogWarning("Source node can have no more out connections.", "Editor", sourceNode);
					return false;
				}
			}
			...
			if (refConnection == null || refConnection.targetNode != targetNode){
				if (targetNode.maxInConnections <= targetNode.inConnections.Count && targetNode.maxInConnections != -1){
					CityParadoxNotion.Services.Logger.LogWarning("Target node can have no more in connections.", "Editor", targetNode);
					return false;
				}
