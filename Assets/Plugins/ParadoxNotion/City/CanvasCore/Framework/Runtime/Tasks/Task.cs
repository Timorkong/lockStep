	...
	abstract public partial class Task : ISubParametersContainer {
		...
		public void SetOwnerSystem(ITaskSystem newOwnerSystem){
			...
			if (newOwnerSystem == null){
                CityParadoxNotion.Services.Logger.LogError("ITaskSystem set in task is null!!", "Init", this);
				return;
			}
		...
		public string summaryInfo{
			get
			{
				#if UNITY_EDITOR
				{
					if (!CityNodeCanvas.Editor.Prefs.showTaskSummary){
		...
		protected bool Error(string error){
            CityParadoxNotion.Services.Logger.LogError(string.Format("{0} | TASK DISABLED | {1}", error, ownerSystem != null? ownerSystem.contextObject : null ), "Task", this);
			return false;
		}
